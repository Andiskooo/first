      ...
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row items-stretch w-full gap-8">
          ...
          <div className="w-full md:w-1/2 lg:w-5/12">
            <h2 className="text-3xl font-bold text-white mb-8">{t('home.blog.sectionTitle', title)}</h2>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h3 className="font-medium text-white">{t(`blog.posts.${post.id}.title`, post.title)}</h3>
                        {post.subtitle && (
                          <p className="text-sm text-gray-300 mt-1">{t(`blog.posts.${post.id}.subtitle`, post.subtitle)}</p>
                        )}
                      </div>
                    </div>
                  </button>
            ...
            >
              {t('home.blog.viewAll', 'Shiko artikujt')}
            </Link>
          </div>
              ...
              <AnimatePresence initial={false} custom={direction} mode="wait">
                ...
                >
                  <div className="rounded-none overflow-hidden h-full">
                      ...
                      <div className="absolute inset-0 flex items-center justify-start p-12">
                        <div className="max-w-md bg-black/50 p-6 rounded-md">
                          <h2 className="text-4xl font-bold text-white mb-4">
                            {t(`blog.posts.${activePost.id}.title`, activePost.title)}
                          </h2>
                          <p className="text-white/90">
                            {t(`blog.posts.${activePost.id}.subtitle`, activePost.subtitle || activePost.content.substring(0, 100) + '...')}
                          </p>
                        </div>
                      </div>
                    ...
                    >
                      {t('home.blog.readMore', 'Lexo më shumë')}
                    </Link>
                  </div>
                </motion.div>
              </AnimatePresence>
